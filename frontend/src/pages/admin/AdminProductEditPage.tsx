...
import { useAdminProduct } from '../../api/admin';

export function AdminProductEditPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { data, isLoading: isFetching } = useAdminProduct(id!);
  ...
  useEffect(() => {
    if (data) {
      ...
      setSummaryDesc(data.summary_desc || '');
      ...
      if (data.items && data.items.length > 0) {
        setItems(data.items.map((item: any) => ({
          ...
          file: null // cannot easily prepopulate file inputs, we leave it null. User can upload a new one to replace.
        })));
      }
    }
  }, [data]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Failed to update product. Check console.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-accent mb-2">Edit Product</h1>
        <p className="text-muted-foreground">Update the details below to edit your product.</p>
      </div>
